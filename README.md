...
Every claim in this README was checked against a real, running build on an actual machine — not a sandboxed or stubbed environment. cache-engine (zero external dependencies beyond Jackson) was built and tested with the full Maven/JUnit toolchain; every number above (hit rates, evictions, latency, the TinyLFU scan-resistance table, the ARC invariant stress test, the adaptive switching event) came from a real run of BenchmarkRunner, not an estimate. cache-api (Spring Boot) and frontend (React) were run end-to-end with the real Spring Boot and npm toolchains — mvn spring-boot:run serving REST/WebSocket on :8080, npm run dev serving the dashboard on :5173 — confirming the full request path from a curl/browser action through the live cache engine and back out over the WebSocket to the dashboard. Run mvn clean install, mvn spring-boot:run, and npm install && npm run dev yourself to reproduce.
